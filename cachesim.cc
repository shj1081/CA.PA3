...
static void help() {
  ...
  std::cerr << "where sets, ways, and blocksize are positive integers, with"
            << std::endl;
  std::cerr << "sets and blocksize both powers of two and blocksize at least 8."
            << std::endl;
  exit(1);
}
...
void cache_sim_t::print_stats() {
  ...
  float mr =
      100.0f * (read_misses + write_misses) / (read_accesses + write_accesses);
...
uint64_t* cache_sim_t::check_tag(uint64_t addr) {
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx * ways + i] & ~DIRTY)) return &tags[idx * ways + i];
...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store) {
  ...
  if (likely(hit_way != NULL)) {
    size_t idx = (addr >> idx_shift) & (sets - 1);
    size_t updated_way = (hit_way - &tags[idx * ways]) / sizeof(uint64_t);
    for (size_t i = 0; i < ways; i++) {
